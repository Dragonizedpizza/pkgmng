...
export const Pacman = {
	async install(packages: string[], options: PacmanInstallOptions) {
			...
			addFlag = (
				{
					...
					v,
					e = true,
				}: { k: string; v?: string; e?: any } = {} as any,
			) => {
				if (!e) return;
		...
		addFlag({
			k: PacmanFlags.databasePath,
			v: options.databasePath,
			e: options.databasePath,
		});
		addFlag({
			k: PacmanFlags.NoDependencyChecks,
			e: options.noDependencyChecks,
		});
